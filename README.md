...
- **仅语音控制**（可选）：[`ffmpeg`](https://ffmpeg.org) 在 `PATH` 中，以及用于语音转文字的 **单独** xAI API 密钥（按量计费，约 $0.10/小时 — CLI 登录 **不** 覆盖此项）。见 [功能与能力](#功能与能力) 中的 **语音控制**。
...
<details>
<summary><strong>语音控制</strong> — 免提听写与实时转写</summary>

输入区的 **麦克风按钮** 用于语音听写，由 [xAI 语音转文字 API](https://docs.x.ai/developers/model-capabilities/audio/voice) 转写。点击后等待蓝色聆听波形，然后说话 — 文字会边说边出现。说 **「grok send」** 可免提提交并继续听下一条（Grok 回复时也可继续口述；消息会排队，结束后一并发送）。再点麦克风可停止并保留进行中的文字。

两词发送短语是刻意设计（不会因消息末尾仅有 “send” 而触发），可通过 `grok.voiceSendPhrase` 配置。默认流式转写；设 `grok.voiceStreaming: false` 可改为一次性批处理。

> **费用：** 语音转文字是 *独立* 的按量 xAI 产品 — 批处理 **$0.10/小时**，流式 **$0.20/小时**，按音频时长计费。实践中约 500 词 ≈ ½–1¢；重度 1 万词日 ≈ 10¢。需要单独的 [console.x.ai](https://console.x.ai) 密钥（`grok.voiceApiKey` / `GROK_VOICE_API_KEY` / `XAI_API_KEY`）— SuperGrok 订阅 **不** 包含 API 额度。为何绕过 CLI、费用如何端到端测得：[research/voice-input.md](research/voice-input.md)。

![输入区中带实时转写的语音控制](docs/screenshots/voice_mode.png)
...
<details>
<summary><strong>成本控制</strong> — token 甜甜圈、<code>/compact</code> 与力度</summary>

**上下文甜甜圈** 在每轮后跟踪用量 — 点击查看精确计数（跨 `/compact` 与恢复仍准确）。**`/compact`**（齿轮 → 压缩对话）压缩过长对话；**+** 开始新会话。
...
| `grok.showTurnMetrics` | `true` | 每轮结束后在页脚显示首字耗时、对话耗时与 token/s。悬停看 token 明细。可在齿轮 → 配置与调试中切换。 |
...
| `grok.voiceApiKey` | `""` | 语音转文字用的 xAI API 密钥 — 单独的 [console.x.ai](https://console.x.ai) 开发者密钥，不是 CLI 登录。空 = 回退到工作区 `.env` 中的 `GROK_VOICE_API_KEY` / `XAI_API_KEY`。 |
| `grok.ffmpegPath` | `""` | 麦克风录音用的 `ffmpeg` 路径。空 = 使用 PATH 中的 `ffmpeg`。 |
| `grok.voiceInputDevice` | `""` | 麦克风设备覆盖。空 = 系统默认（Windows 自动检测首个 DirectShow 音频设备）。 |
| `grok.voiceSendPhrase` | `"grok send"` | 转写末尾匹配时自动提交的口述短语。空 = 禁用免提发送。 |
| `grok.voiceStreaming` | `true` | 边说边流式转写。`false` = 一次性批处理。流式 $0.20/小时，批处理 $0.10/小时。 |
